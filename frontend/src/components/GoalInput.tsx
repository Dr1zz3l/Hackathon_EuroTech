 ...
 *
 * On submit:
 *   1. Calls POST /api/parse-goal
 *   2. Builds a synthetic Scenario from the response
 *   3. Calls onScenario(scenario) → triggers map recolour
 *
...
import { buildSyntheticScenario, parseGoal } from '../lib/llm'
...
export default function GoalInput({ onScenario }: GoalInputProps) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const parsed   = await parseGoal(trimmed, locale as Locale)
      const scenario = buildSyntheticScenario(parsed)
      onScenario(scenario)
      // Keep the text so the user can see what they typed
    } catch {
  ...
  return (
    <form
      ...
      className="flex flex-col gap-1 px-3 py-2 bg-slate-800 border-b border-slate-700"
    >
